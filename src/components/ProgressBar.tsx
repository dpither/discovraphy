...
export default function ProgressBar({
  ...
  const percent = isDragging
    ? Math.max(0, Math.min(100, seekTarget.current / Math.max(durationMs, 1)) * 100)
  ...
  return (
    <div className="flex w-full flex-col">
      ...
      >
        <div
          ref={progressBarRef}
          className="bg-sub-text-light dark:bg-sub-text-dark relative h-1 flex-1 cursor-pointer rounded-full"
        >
          {/* filled progress */}
          <div
            className="bg-blue w- absolute top-0 bottom-0 left-0 rounded-full"
            style={{ width: `${percent}%` }}
          />
          {/* thumb */}
          <div
            className={`absolute top-1/2 size-2 -translate-y-1/2 rounded-full ${isDragging ? "bg-hover-blue" : "group-hover:bg-blue bg-transparent"}`}
            style={{
      ...
      {/* time labels */}
      <div className="text-sub-text-light dark:text-sub-text-dark flex justify-between text-xs">
